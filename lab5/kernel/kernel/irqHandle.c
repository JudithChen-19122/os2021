...
void syscallOpen(struct StackFrame *sf) {
	...
	if (ret == 0) { // file exist
		...
		// when in use fail to open
		for (i = 0; i < MAX_FILE_NUM; i ++) {
			if (file[i].state == 1) { //in use
		...
		if ((sf->edx >> 3) % 2 == 0) { 
			...
			ret = allocInode(&sBlock, gDesc, &fatherInode, fatherInodeOffset, &destInode, &destInodeOffset, str + length + 1, REGULAR_TYPE);
			...
			ret = allocInode(&sBlock, gDesc, &fatherInode, fatherInodeOffset, &destInode, &destInodeOffset, str + length + 1, DIRECTORY_TYPE);		
		}
		if (ret == -1) {
			pcb[current].regs.eax = -1;
			return;
...
void syscallWriteFile(struct StackFrame *sf) {
	...
	while(has_wrote < size){
		if(cur > inode.blockCount){
			ret = allocBlock(&sBlock, gDesc, &inode, file[sf->ecx - MAX_DEV_NUM].inodeOffset);
			if (ret < 0) // don't have free block
			{
				inode.size = old_off + has_wrote > inode.size ? old_off + has_wrote : inode.size;
				sf->eax = has_wrote; 
		...
		ret = readBlock(&sBlock, &inode, cur, buffer);
		int block_left = sBlock.blockSize - start;
		if(block_left >= size - has_wrote){
			for (int k = 0; k < size - has_wrote; k++)
			{
				buffer[j] = str[i++];
			}
			break;
			...
			file[sf->ecx - MAX_DEV_NUM].offset += block_left;
			has_wrote += block_left;
			cur++;
			start = 0;
			j = start;
			ret = readBlock(&sBlock, &inode, cur, buffer);
	...
	diskWrite(&inode, sizeof(Inode), 1, file[sf->ecx - MAX_DEV_NUM].inodeOffset);
	pcb[current].regs.eax = size;
	file[sf->ecx - MAX_DEV_NUM].offset += size - has_wrote;
	return;
}
...
void syscallReadFile(struct StackFrame *sf) {
	...
	j=start;
        int cur = quotient; 
	int initoffset = file[sf->ecx - MAX_DEV_NUM].offset;       
	...
	while(has_read < size){
		int block_left = sBlock.blockSize - start;
		if (block_left + file[sf->ecx - MAX_DEV_NUM].offset >= inode.size)
		{
			...
			has_read +=inode.size - file[sf->ecx - MAX_DEV_NUM].offset;
			file[sf->ecx - MAX_DEV_NUM].offset = inode.size;
			pcb[current].regs.eax = has_read;
			return;
		}
		if(block_left >= size - has_read){
			for (int k = 0; k < size - has_read; k++)
			{
				str[i] = buffer[j++];
				i++;
			}
	...
	pcb[current].regs.eax = has_read;
	file[sf->ecx - MAX_DEV_NUM].offset = initoffset + has_read;
	return;
}
...
void syscallLseek(struct StackFrame *sf) {
	...
	switch(sf->ebx) { // whence
		case SEEK_SET:
			...
			pcb[current].regs.eax = file[i-MAX_DEV_NUM].offset;
			return;
			break;
		case SEEK_CUR:
			// TODO: if SEEK_CUR
